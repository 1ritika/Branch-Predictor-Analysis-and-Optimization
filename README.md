...
```
git clone https://github.com/casper-iitbombay/ChampSim-TimeCache
```
...
Execute `champ_script.sh` which contains the commands to execute on each trace file

It has commands to execute 'run_champsim.sh' with proper input arguments. The default `TRACE_DIR` in `run_champsim.sh` is set to `$PWD/Traces` <br>
